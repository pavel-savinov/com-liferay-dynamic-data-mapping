		...
		var LinkToPageField = A.Component.create(
			{
				...
				prototype: {
					...
					_getLinkToPageItemSelectorURL: function() {
						var instance = this;
						...
						var criterionJSON = {
							desiredItemSelectorReturnTypes: 'com.liferay.item.selector.criteria.UUIDItemSelectorReturnType'
						};
					...
					_openLinkToPageModal: function() {
						...
						var layoutNameNode = container.one('#' + instance.getInputName() + 'LayoutName');
						var layoutJSONNode = instance.getInputNode();

						var itemSelectorDialog = new A.LiferayItemSelectorDialog(
							{
								eventName: portletNamespace + 'selectLinkToPage',
								on: {
									selectedItemChange: function(event) {
										var selectedItem = event.newVal;

										if (selectedItem) {
											layoutNameNode.val(selectedItem.name);
											layoutJSONNode.val(JSON.stringify(selectedItem));
										}
									}
								},
